...
def keyword_extractor_agent(state: AppState) -> AppState:
    ...
    query_map = {
        "슬픔": f"우울 슬픔 극복 심리 상담 {top_3_keywords}",
        "우울": f"우울증 극복 마음 치유 {top_3_keywords}",
        "불안": f"불안 해소 스트레스 관리 {top_3_keywords}",
        "걱정": f"걱정 해소 심리 안정 {top_3_keywords}",
        "분노": f"분노 조절 감정 관리 {top_3_keywords}",
        "화": f"화 조절 갈등 해결 {top_3_keywords}",
        "기쁨": f"행복 유지 긍정 에너지 {top_3_keywords}",
        "행복": f"행복한 삶 긍정 마인드 {top_3_keywords}",
        "사랑": f"관계 유지 사랑 키우기 {top_3_keywords}",
    }
    
    content_query = query_map.get(dominant, f"감정 관리 심리 건강 {top_3_keywords}")
